...
mydb = sqlite3.connect('static/corpus.db', check_same_thread=False)
...
def check(inputText):
    ...
    for bookID in range(1, 201):
        ...
        mycursor = mydb.execute(
            "SELECT MAX(PAGENO) FROM PAGE WHERE ID="+str(bookID)+";")
        totalpages = int(mycursor.fetchone()[0])
        for page in range(1, totalpages+1):
            mycursor = mydb.execute(
                "SELECT CONTENT FROM PAGE WHERE ID="+str(bookID)+" AND PAGENO="+str(page)+";")
            dbtext = mycursor.fetchone()[0]

            dbtokens = dbtext.split(" ")
            # Removing stopwords
            dbtokens = [i for i in dbtokens if i not in bangla_stopwords]
